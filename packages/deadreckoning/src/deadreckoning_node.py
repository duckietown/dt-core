...
class DeadReckoningNode(DTROS):
    ...
    def __init__(self, node_name):
        ...
        #self.left_time_last = None
        #self.right_time_last = None
        ...
        #self.ts_encoders = message_filters.TimeSynchronizer([self.sub_encoder_left, self.sub_encoder_right], 10)
        ...
        rospy.loginfo("[deadreckoning_node] Initialized")
    ...
    def cb_ts_encoders (self, left_encoder, right_encoder):
        ...
        # Update encoder values no faster than specified frequency
        #if (timestamp - self.encoders_timestamp_last) < 1.0/MAX_ENCODER_HZ:
        #    return
        ...
        if dtl.to_sec() < 0 or dtr.to_sec() < 0:
            rospy.loginfo("[deadreckoning_node] Ignoring stale encoder message")
            return
        ...
        if self.debug:
            rospy.loginfo ("[%s] Left wheel:\t Time = %.4f\t Ticks = %d\t Distance = %.4f m", self.node_name, left_encoder.header.stamp.to_sec(), left_encoder.data, left_distance)

            rospy.loginfo ("[%s] Right wheel:\t Time = %.4f\t Ticks = %d\t Distance = %.4f m", self.node_name, right_encoder.header.stamp.to_sec(), right_encoder.data, right_distance)

            rospy.loginfo("[%s] TV = %.2f m/s\t RV = %.2f deg/s\t DT = %.4f", self.node_name, self.tv, self.rv*180/math.pi, dt)
    ...
    def cb_timer (self, event):
        ...
        if self.encoders_timestamp_last:
            ...
            if abs(dt) > self.encoder_stale_dt:
                rospy.loginfo("[%s] No encoder messages received for %.2f seconds. Setting translational and rotational velocities to zero", self.node_name, dt)
                self.rv = 0.0
                self.tv = 0.0
        else:
            rospy.loginfo("[%s] No encoder messages received. Setting translational and rotational velocities to zero", self.node_name)
            self.rv = 0.0
            self.tv = 0.0
...
if __name__ == '__main__':
    ...
    # create node
    node_name = "deadreckoning_node"
    node = DeadReckoningNode(node_name)
    rospy.loginfo("[%s] Spinning up", node_name)
    rospy.spin()
