...
# -------- Start adding back the tag info stuff
        ...
        self.pkg_path = rospack.get_path('apriltags2_ros')
        # No tags_file input atm., so default value is used
        tags_filepath = self.setupParam(
            "~tags_file", self.pkg_path+"/../signs_and_tags/apriltagsDB.yaml")
...
# ---- end tag info stuff
        ...
        self.sub_prePros = rospy.Subscriber(
            "~apriltags_in", AprilTagDetectionArray, self.callback, queue_size=1)
        self.pub_postPros = rospy.Publisher(
            "~apriltags_out", AprilTagsWithInfos, queue_size=1)
        self.pub_visualize = rospy.Publisher(
            "~tag_pose", PoseStamped, queue_size=1)
    ...
    def callback(self, msg):
        ...
        for detection in msg.detections:
            ...
            veh_R_camxout = tr.euler_matrix(
                0, self.camera_theta*np.pi/180, 0, 'rxyz')
            veh_T_camxout = tr.concatenate_matrices(
                veh_t_camxout, veh_R_camxout)   # 4x4 Homogeneous Transform Matrix

            camxout_T_camzout = tr.euler_matrix(-np.pi/2, 0, -np.pi/2, 'rzyx')
            veh_T_camzout = tr.concatenate_matrices(
                veh_T_camxout, camxout_T_camzout)
            ...
            veh_T_tagxout = tr.concatenate_matrices(
                veh_T_camzout, camzout_T_tagzout, tagzout_T_tagxout)
            ...
            new_tag_data.detections.append(AprilTagDetection(int(detection.id[0]), float(
                detection.size[0]), PoseStamped(header, Pose(trans, rot))))
