...
class LegacyGroundProjectionGeometry(object):
    ...
    def __init__(self, camera_info, homography):
        self.ci = camera_info
        self.H = homography
        self.new_gpg = GroundProjectionGeometry(self.ci.width, self.ci.height, self.H)
        self.new_rectifier = Rectify(self.ci)

    def get_camera_info(self):
        return self.ci

    def pixel2vector(self, pixel):
        return self.new_gpg.pixel2vector(pixel)

    def ground2pixel(self, point):
        return self.new_gpg.ground2pixel(point)

    def rectify(self, cv_image_raw, interpolation=cv2.INTER_NEAREST):
        return self.new_rectifier.rectify(cv_image_raw, interpolation)

    def distort(self, rectified):
        return self.new_rectifier.distort(rectified)
