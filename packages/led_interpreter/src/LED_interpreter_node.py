...
class LEDInterpreterNode(object):
    def __init__(self):
        ...
        rospy.loginfo('[%s] Intersection Type: %s' %
                      (self.node_name, rospy.get_param("~intersectionType")))
        ...
        self.signalFrequencies = [self.carSignalA]
        self.vehicleSignals = [SignalsDetection.SIGNAL_A]
        ...
        self.pub_interpret = rospy.Publisher(
            "~signals_detection", SignalsDetection, queue_size=1)
        #self.sub_tags = rospy.Subscriber("apriltags_postprocessing_fast_node/apriltags", AprilTags, self.CheckTags)
        self.sub_LEDs = rospy.Subscriber(
            "~raw_led_detection", LEDDetectionArray, self.Interpreter, queue_size=1)
    ...
    def Interpreter(self, msg):
        ...
        if self.trafficLightIntersection:
            for item in msg.detections:
                rospy.loginfo("[%s]:\n pixel = %f\n top bound = %f\n measured frequence=%f\n go frequency =%f" % (
                    self.node_name, item.pixels_normalized.y, self.label['top'], item.frequency, self.lightGo))
        ...
        self.pub_interpret.publish(SignalsDetection(
            front=self.front, right=self.right, left=self.left, traffic_light_state=self.traffic_light_state))
