...
def jobs_videos(context, log, name, outd, only_camera):
    ...
    for topic in topics:
        ...
        actual_count, count, _stopped_early = count_messages_in_slice(
        ...
        if only_camera:
            ...
            j = context.comp(
                dtu.d8n_make_video_from_bag, filename, topic, out, job_id="%s-%s" % (name, topic)
            )
