class Detections:
    ...
    def __init__(self, lines, normals, centers, map):
        ...
        self.normals = (
            normals  #: An ``Nx2`` array with every row representing the normal of a line ``[nx, ny]``
        )
        self.centers = (
            centers  #: An ``Nx2`` array with every row representing the center of a line ``[cx, cy]``
        )
