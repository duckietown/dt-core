        ...
        if self.unfiltered_leds:
            for d in self.unfiltered_leds.detections:
                dist = (d.pixels_normalized.x * W - click_img_coord.x()) ** 2 + (
                        d.pixels_normalized.y * H - click_img_coord.y()) ** 2
                if (dist < mindist and
...
class LEDVisualizerNode:
    def __init__(self):
        ...
        self.sub_info = rospy.Subscriber("/" + self.veh_name + "/led_detector_node/debug_info",
                                         LEDDetectionDebugInfo, self.info_callback)
        self.sub_info = rospy.Subscriber("/" + self.veh_name + "/camera_node/image/compressed",
                                         CompressedImage, self.cam_callback)
        self.sub_info = rospy.Subscriber("/" + self.veh_name + "/led_detector_node/raw_led_detection",
                                         LEDDetectionArray, self.result_callback)
        self.pub_trigger = rospy.Publisher("/" + self.veh_name + "/led_detector_node/trigger", Byte,
                                           queue_size=1)
