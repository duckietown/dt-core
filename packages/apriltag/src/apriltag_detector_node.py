...
class AprilTagDetector(DTROS):
    def __init__(self):
        ...
        self._tag_pub = rospy.Publisher(
            "tag_detections", AprilTagDetectionArray, queue_size=1, dt_topic_type=TopicType.PERCEPTION
        )
    ...
    def _img_cb(self, data):
        ...
        for tag in tags:
            ...
            self._tf_bcaster.sendTransform(
                p.tolist(), q.tolist(), detection_time, "/tag{:s}".format(str(tag.tag_id)), self._camera_frame
            )
        # publish detections
