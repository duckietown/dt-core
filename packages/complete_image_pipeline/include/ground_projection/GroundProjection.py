...
from duckietown_utils import (get_duckiefleet_root, logger)
...
class GroundProjection():
    ...
    def rectify(self, cv_image_raw):
        ...
        mapx, mapy = cv2.initUndistortRectifyMap(self.pcm_.K, self.pcm_.D, self.pcm_.R, self.pcm_.P,
                                                 (self.pcm_.width, self.pcm_.height), cv2.CV_32FC1, mapx,
                                                 mapy)
    ...
    def estimate_homography(self, cv_image):
        ...
        ret, corners = cv2.findChessboardCorners(cv_image_rectified,
                                                 (self.board_['width'], self.board_['height']),
        ...
        for r in range(self.board_['height']):
            for c in range(self.board_['width']):
                src_pts.append(np.array([r * self.board_['square_size'], c * self.board_['square_size']],
        ...
        extrinsics_filename = get_duckiefleet_root() + "/calibrations/camera_extrinsic/" + self.robot_name \
                              + ".yaml"
    ...
    def load_board_info(self, filename=''):
        ...
        if not os.path.isfile(filename):
            filename = get_ros_package_path(
                'duckietown') + '/config/baseline/ground_projection/ground_projection/default.yaml'
    ...
    def load_camera_info(self):
        ...
        filename = (os.environ[
                        'DUCKIEFLEET_ROOT'] + "/calibrations/camera_intrinsic/" + self.robot_name + ".yaml")
        if not os.path.isfile(filename):
        ...
        logger.info("Loaded camera calibration parameters for {} from {}".format(self.robot_name,
